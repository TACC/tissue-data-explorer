...
4. Replace the variables in the code snippet below with your username and the appropriate version tag and run the commands to publish the images to Docker Hub
...
5. On the production server, pull the newly published images from Docker Hub
...
6. Copy the `docker-compose-prod.yaml` file and your `.env` file for the production configuration app onto the production server. You will need to make the following changes to `docker-compose-prod.yaml`:
...
7. Set up the shared volume. You can use the `./build/create_volume.sh` script to do this. If you are starting from scratch, you can use the data in the `data/start` folder to see a demo version of the app.
...
8. Run the image. The production containers should restart automatically if the server reboots due to the Docker restart policy.

   ```
   USERID=${UID} GROUPID=${GID} docker compose -f docker-compose-prod.yaml up
   ```

9. Clean up old images by running the `./build/clean_images.sh` script. This script will remove all containers and images with "tde-" in the name, then clean the Docker build cache.
